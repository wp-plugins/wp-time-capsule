...
class Dropbox_OAuth_Consumer_Curl extends Dropbox_OAuth_Consumer_ConsumerAbstract
{
   ...
   /**
     ...
     */
    public function fetch($method, $url, $call, $additional = array(), $isChunkDownload = array())
    {
        ...
        if ($method == 'GET' && $this->outFile) { // GET
			...
			if(!empty($isChunkDownload))
			{
				...
				//file_put_contents(WP_CONTENT_DIR .'/DE_clientPluginSIde.php',"\n ----setting range------- ".var_export($this_range,true)."\n",FILE_APPEND);
				//curl_setopt($ch, CURLOPT_RANGE, ''.$c_offset.'-'.$c_limit.'');	
			}
		...
		////file_put_contents(WP_CONTENT_DIR .'/DE_clientPluginSIde.php',"\n ----response on curl------ ".var_export($response,true)."\n",FILE_APPEND);
