...
class Dropbox_API
{
    ...
    */
     ...
     * @return array
     */
    public function getFile($file, $outFile = false, $revision = null, $isChunkDownload = array())
    {
		//file_put_contents(WP_CONTENT_DIR .'/DE_clientPluginSIde.php',"\n -----restoring as single file------ ".var_export($outFile,true)."\n",FILE_APPEND);
        $handle = null;
		$tempFolder = $this->getTempFolderFromOutFile(stripslashes($outFile));
		if ($outFile !== false) {
        ...
        $response = $this->fetch('GET', self::CONTENT_URL, $call, $params, $isChunkDownload);
    ...
    {
		$tempFolderFile = $this->getTempFolderFromOutFile(stripslashes($outFile));
		//file_put_contents(WP_CONTENT_DIR .'/DE_clientPluginSIde.php',"\n -----this is the temp folder ------ ".var_export($tempFolderFile,true)."\n",FILE_APPEND);
		
		//setting chmod from filesystem
		global $wp_filesystem;
		$chRes = $wp_filesystem->chmod($tempFolderFile, false, true);
		
		//file_put_contents(WP_CONTENT_DIR .'/DE_clientPluginSIde.php',"\n ----chmod result------ ".var_export($chRes,true)."\n",FILE_APPEND);
	...
	public function getTempFolderFromOutFile($outFile, $mode = '')
    {
		...
		//file_put_contents(WP_CONTENT_DIR .'/DE_clientPluginSIde.php',"\n -----out file is------ ".var_export($outFile,true)."\n",FILE_APPEND);
		//file_put_contents(WP_CONTENT_DIR .'/DE_clientPluginSIde.php',"\n ----supposed abspath file path------- ".var_export(ABSPATH,true)."\n",FILE_APPEND);
		$this_absbath_length = (strlen(ABSPATH) - 1);
		$this_temp_file = WP_CONTENT_DIR . '/tCapsule' . substr($outFile, $this_absbath_length);
		
		//file_put_contents(WP_CONTENT_DIR .'/DE_clientPluginSIde.php',"\n ----supposed tempFIle----- ".var_export($this_temp_file,true)."\n",FILE_APPEND);
		...
		//file_put_contents(WP_CONTENT_DIR .'/DE_clientPluginSIde.php',"\n ----supposed folder----- ".var_export($this_temp_folder,true)."\n",FILE_APPEND);
		//file_put_contents(WP_CONTENT_DIR .'/DE_clientPluginSIde.php',"\n ---folder basename--- ".var_export(basename($this_temp_folder),true)."\n",FILE_APPEND);
	...
	public function createRecursiveFileSystemFolder($this_temp_folder, $this_absbath_length = null){
		//create the folder if not exists by fileSystemMethod; this functions is recursive if the current parent folder is not created then it is created
		//file_put_contents(WP_CONTENT_DIR .'/DE_clientPluginSIde.php',"\n ----recursive tempFOlder------- ".var_export($this_temp_folder,true)."\n",FILE_APPEND);
		global $wp_filesystem;
		if ( !$wp_filesystem->is_dir($this_temp_folder) ) {
			if ( !$wp_filesystem->mkdir($this_temp_folder, FS_CHMOD_DIR) )
				{
					//calculate the parent folder and try creating it
					$this_folder_basename_pos = strrpos($this_temp_folder, basename($this_temp_folder));
					$this_parent_folder = substr($this_temp_folder, 0, $this_folder_basename_pos);
					
					$this->createRecursiveFileSystemFolder($this_parent_folder);
				}
				else
				{
					//file_put_contents(WP_CONTENT_DIR .'/DE_clientPluginSIde.php',"\n -----folder creation success------ ".var_export($this_temp_folder,true)."\n",FILE_APPEND);
				}
		}
	}
     ...
     * @return array
     */
    public function chunkedDownload($file, $outFile = false, $revision = null, $isChunkDownload = array())
    {
		//file_put_contents(WP_CONTENT_DIR .'/DE_clientPluginSIde.php',"\n ----doing chunked download------ ".var_export($outFile,true)."\n",FILE_APPEND);
        $handle = null;
        if ($outFile !== false) {
            // Create a file handle if $outFile is specified
		...
		$outFilePath = $this->getTempFolderFromOutFile(stripslashes($outFile));
		//file_put_contents(WP_CONTENT_DIR .'/DE_clientPluginSIde.php',"\n ------outFilePath------- ".var_export($outFilePath,true)."\n",FILE_APPEND);
        $file = $this->encodePath($file);
        $call = 'files/' . $this->root . '/' . $file;
        $params = array('rev' => $revision);
        $response = $this->fetch('GET', self::CONTENT_URL, $call, $params, $isChunkDownload);
		//file_put_contents(WP_CONTENT_DIR .'/DE_clientPluginSIde.php',"\n ----chunked download response------- ".var_export($response,true)."\n",FILE_APPEND);
		...
		//file_put_contents(WP_CONTENT_DIR .'/DE_clientPluginSIde.php',"\n -----offset before------- ".var_export($offset,true)."\n",FILE_APPEND);
		if ($this->tracker) {
    ...
    private function fetch($method, $url, $call, array $params = array(), $isChunkDownload = array())
    {
        ...
        return $this->OAuth->fetch($method, $url, $call, $params, $isChunkDownload);
    }
