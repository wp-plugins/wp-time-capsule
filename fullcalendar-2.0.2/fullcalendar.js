		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					if (buttonName == 'title') {
						e.append("<span class='fc-header-title'><h2 style='padding:0;'>&nbsp;</h2></span>");
						if (prevButton) {
							...
							var button = $(
								...
								.click(function() {
									if (!button.hasClass(tm + '-state-disabled')) {
										//timeCapsule - to stop the reloading 
...
function BasicView(element, calendar, viewName) {
	...
	function buildCellHTML(date) { // date assumed to have stripped time
		...
		html +=
			...
			//"<div> "+htmlEscape(t.getEventTimeText(event)) +" </div>" +
			"</div>" +
			"</div>" +
			"</td>";
	...
	function setHeight(height) {
		...
		bodyFirstCells.each(function(i, _cell) {
			if (i < rowCnt) {
				cell = $(_cell);
				cell.css(
					'height', ((jQuery(window).height() - 250) / 6) + 'px'
...
function DayEventRenderer() {
	...
	function _renderDayEvents(events, doAppend, doRowHeights) {
		...
		//timeCapsule events
		jQuery(".fc-event-container").hide();
		//jQuery(".fc-header-right span").hide();
		jQuery(".fc-button-month, .fc-button-agendaWeek, .fc-button-agendaDay").hide();
	...
	function buildHTMLForSegment(segment) {
		//the following block of code wont run
		var html = '';
		var isRTL = opt('isRTL');
		...
		html +=
			"<span class='fc-event-title' backupIDs='"+htmlEscape(event.backupIDs || '')+"' >" +
			htmlEscape(event.title || '') +
			"</span>" +
			"</div>";
		...
		//timeCapsule
		jQuery('.fc-day').each(function(k, v){
