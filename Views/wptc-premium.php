...
if (isset($_POST['name'])) {
    try {
        ...
        add_settings_error('error', 'wptc_premium_error', $e->getMessage(), 'error');
    }
}
...
function wptc_products($manager, $type, $extensions)
{
            ...
            <?php else: ?>
                <div class="product-box__button">
                    <form action="<?php echo is_int($extension['expiry']) ? $installUrl : $buyUrl ?>" method="post" id="extension-<?php echo esc_attr($extension['name']) ?>">
                        <input type="hidden" value="<?php echo WPTC_Extension_Manager::API_KEY ?>" name="apikey" />
