...
class WPTC_DropboxFacade
{
	...
	public function download_file($path, $file, $revision = '', $isChunkDownload = null)
    {
		$i = 0;
        while ($i++ < self::RETRY_COUNT) {
            try {
                //return $this->dropbox->getFile($path, $this->remove_secret($file), $revision);
	...
	public function chunk_download_file($path, $file, $revision = '', $isChunkDownload = null)
    {
		//$isChunkDownload = array();
		//get the filesize and see whether chunked download or normal download 					//manual
        $i = 0;
        while ($i++ < self::RETRY_COUNT) {
            try {
                return $this->dropbox->chunkedDownload($path, $file, $revision, $isChunkDownload);
            } catch (Exception $e) {}
        }
        throw $e;
    }
