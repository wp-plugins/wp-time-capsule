...
class WPTC_FileList
{
    ...
    public function __construct()
    {
        ...
        $result = $this->db->get_results("SELECT * FROM {$this->db->prefix}wptc_excluded_files WHERE isdir = 0");
        foreach ($result as $value) {
            $this->excluded_files[] = $value->file;
        }

        $result = $this->db->get_results("SELECT * FROM {$this->db->prefix}wptc_excluded_files WHERE isdir = 1");
        foreach ($result as $value) {
    ...
    private function exclude_file($file)
    {
        if (!in_array($file, $this->excluded_files)) {
            ...
            $this->db->insert("{$this->db->prefix}wptc_excluded_files", array(
                'file' => $file,
                'isdir' => false
            ));
        }
    ...
    private function include_file($file)
    {
        ...
        $this->db->query(
            $this->db->prepare("DELETE FROM {$this->db->prefix}wptc_excluded_files WHERE file =  %s", $file)
        );
    ...
    private function include_dir($dir)
    {
        ...
        $this->db->query(
            $this->db->prepare("DELETE FROM {$this->db->prefix}wptc_excluded_files WHERE file =  %s", $dir)
        );
