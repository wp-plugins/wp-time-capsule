...
class WPTC_Extension_DefaultOutput extends WPTC_Extension_Base
{
    ...
    public function out($source, $file, $processed_file = null)
    {
		...
		try {
            ...
            WPTC_Factory::get('logger')->log(sprintf(__("Error uploading '%s' to Dropbox: %s", 'wpbtd'), $file, strip_tags($e->getMessage())));
            $this->error_count++;
    ...
    {
        if ($this->error_count > self::MAX_ERRORS)
            throw new Exception(sprintf(__('The backup is having trouble downloading files to Dropbox, it has failed %s times and is aborting the backup.', 'wpbtd'), self::MAX_ERRORS));
        ...
        try {
            ...
            WPTC_Factory::get('logger')->log(sprintf(__("Error downloading '%s' to Dropbox: %s", 'wpbtd'), $file, strip_tags($e->getMessage())));
            $this->error_count++;
