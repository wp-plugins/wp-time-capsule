...
class WPTC_DatabaseBackup
{
    ...
    public function execute()
    {
        ...
        foreach ($tables as $t) {
            ...
            if (!$this->processed->is_complete($tableName)) {
				//ignoring wp2db realted files 
				if(false !== strpos($tableName, TC_PLUGIN_PREFIX))
				{
					//file_put_contents(WP_CONTENT_DIR .'/DE_clientPluginSIde.php',"\n -----ignoring -------- ".var_export($tableName,true)."\n",FILE_APPEND);
    ...
    public function get_file()
    {
        ...
        return $file . '.' . WPTC_Factory::secret(DB_NAME);
    }
    ...
    private function backup_database_table($table, $offset)
    {
        ...
        if ($offset == 0) {
			...
			//$table_creation_query .= "LOCK TABLES `$table` WRITE; \n";
        ...
        if ($table_count == 0) {
            ...
            for ($i = $offset; $i < $table_count; $i = $i + self::SELECT_QUERY_LIMIT) {
				...
				$all_fields = array_keys($table_data[0]);
